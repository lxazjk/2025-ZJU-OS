...
void task_init(void) {
  ...
  for(int i = 0; i < LAB_TEST_NUM; ++i) {
    ...
    task[i] -> thread.sp = (unsigned long long)(Kernel_Page + PAGE_SIZE * (i + 1));
...
void schedule(void) {
  unsigned char next; int mn = -1;
  // TODO
  for(int i = NR_TASKS - 1; i >= 0; i--) {
    if(!task[i] || task[i] -> state != TASK_RUNNING || task[i] -> counter <= 0) continue;
    if(mn == -1 || task[i] -> counter < mn) {
      mn = task[i] -> counter;
      next = i;  
    }
  }  
...
void schedule(void) {
  unsigned char next; int priority = -1, counter = -1;
  // TODO
  for(int i = NR_TASKS - 1; i >= 0; i--) {
    if(!task[i] || task[i] -> state != TASK_RUNNING || task[i] -> counter <= 0) continue;
    if(priority == -1 || task[i] -> priority < priority) {
      priority = task[i] -> priority; counter = task[i] -> counter; next = i;
    } else if(task[i] -> priority == priority) {
      if(counter < task[i] -> counter) {
