...
#define USER_STACK_BASE 0x1001000
#define PROG_START      0x1000000
...
struct ret_info syscall(uint64_t syscall_num, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t sp) {
    ...
    case SYS_FORK: {
        ...
        for(i = 0; i < NR_TASKS; i++){ 
            if(!task[i]||task[i]->counter==0)break; // DONE: why consider counter == 0 same
        }
        if(i == NR_TASKS) break;
        if(!task[i]) task[i] = (struct task_struct*)(VIRTUAL_ADDR(alloc_page()));
        task[i]->state = TASK_RUNNING;
        task[i]->counter = current->priority;
        task[i]->priority = current->counter;
        task[i]->blocked = 0;
        task[i]->pid = i;
        // 2. create root page table, set current process's satp
        uint64_t root_page_table =  alloc_page();
        task[i]->satp = root_page_table >> 12 | SATP_MODE | (((uint64_t) (task[i]->pid))  << 44);
        ...
        create_mapping((uint64_t*)root_page_table, PROG_START, task[i]->mm.user_program_start, PAGE_SIZE, PTE_V | PTE_R | PTE_X | PTE_U | PTE_W);
        //   2.2 create mapping for kernel address
        create_mapping((uint64_t*)root_page_table, KERNEL_BASE, PHY_BASE, 16 * 1024 * 1024, PTE_V | PTE_R | PTE_W | PTE_X);
        create_mapping((uint64_t*)root_page_table, KERNEL_BASE, PHY_BASE, PHYSICAL_ADDR((uint64_t)&rodata_start) - PHY_BASE, PTE_V | PTE_R | PTE_X);
        ...
        create_mapping((uint64_t*)root_page_table, PHY_BASE, PHY_BASE, PHYSICAL_ADDR((uint64_t)&rodata_start) - PHY_BASE, PTE_V | PTE_R | PTE_X);
        create_mapping((uint64_t*)root_page_table, PHY_BASE, PHY_BASE, 16 * 1024 * 1024, PTE_V | PTE_R | PTE_W | PTE_X);
        ...
        create_mapping((uint64_t*)root_page_table, USER_STACK_BASE, physical_stack, PAGE_SIZE, PTE_V | PTE_R | PTE_W | PTE_U);
    ...
    case SYS_EXEC: {
        ...
        write_csr(sscratch, USER_STACK_BASE + PAGE_SIZE);
        current->mm.user_program_start = get_program_address((char *)arg0);
        // 3. create mapping for new user program address
        create_mapping((uint64_t*)root_page_table, PROG_START, current->mm.user_program_start, PAGE_SIZE, PTE_V | PTE_R | PTE_X | PTE_U | PTE_W);
        // 4. set sepc = 0x1000000
        sp_ptr[16] = PROG_START;
        // 5. refresh TLB
